...
function compareFirstCard(a, b) {
    ...
    const values = "A,K,Q,T,9,8,7,6,5,4,3,2,J".split(",").reverse();
...
function isFiveOfAKind(value) {
    ...
    var result = false;
    var jokerCount = counts['J'] || 0;

    if (jokerCount >= 4) return true;

    cards.forEach((element, i) => {
        if(element === 'J') return;
        if(counts[element] + jokerCount == 5) {
            result = true;
            return
        }
    });
...
function isFourOfAKind(value) {
    ...
    cards.forEach((element, i) => {
        if(element === 'J') return;
        if(counts[element] + jokerCount == 4) {
            result = true;
            return
        }
    });
...
function isThreeOfAKind(value) {
    ...
    cards.forEach((element, i) => {
        if(element === 'J') return;
        if(counts[element] + jokerCount == 3) {
            result = true;
            return
        }
    });
...
function isFullHouse(value) {
    ...
    if(hasThreeOfaKind && hasThreeOfaKind) return true;
    if(jokerCount == 1 && hasThreeOfaKind) return true;
    if(jokerCount == 1 && isTwoPair(value.replace("J", ""))) return true;
    return false;
}
